...
export default function IpEventEditPage() {
  ...
  const getEvent = async () => {
    try {
      ...
      if (!response.ok) {
        setMessage(data.message || '이벤트 정보를 불러오지 못했습니다.');
        return;
      }

      setIpId(String(data.ipId ?? ''));
      setName(data.name ?? '');
      ...
      setMessage('이벤트 조회 중 오류가 발생했습니다.');
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        setMessage(data.message || 'IP 이벤트 수정에 실패했습니다.');
        return;
      }

      alert('IP 이벤트가 수정되었습니다.');
      router.push(`/ip-events/${eventId}`);
    } catch {
  ...
  return (
    <div style={styles.page}>
      <div style={styles.card}>
        <h1 style={styles.title}>IP 이벤트 수정</h1>
